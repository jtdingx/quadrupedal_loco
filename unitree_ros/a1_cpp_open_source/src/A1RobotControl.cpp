...
void A1RobotControl::update_plan(A1CtrlStates &state, double dt) {
    ...
    for (int i = 0; i < NUM_LEG; ++i) {
        ...
        // cout << "state.root_pos"<<state.root_pos <<endl;
        // cout << "state.root_rot_mat"<<state.root_rot_mat <<endl;
        // cout << "state.foot_pos_target_rel"<<state.foot_pos_target_rel <<endl;
    }
}
...
Eigen::Matrix<double, 3, NUM_LEG> A1RobotControl::compute_grf(A1CtrlStates &state, double dt) {
    ...
    if (state.stance_leg_control_type == 0) { // 0: QP
        ...
        double cos_yaw = cos(state.root_euler[2]);
        double sin_yaw = sin(state.root_euler[2]);
        ...
        for (int i = 0; i < PLAN_HORIZON; i++) {
            ...
            if (i ==0)
            {
                // cout<< state.robot_mass <<endl;
                // cout<< state.a1_trunk_inertia <<endl;
                // cout << state.root_rot_mat <<endl;
                // cout << state.foot_pos_abs <<endl;
            }

            //// should update the foot position in the prediction window;
            ...
            // if (i ==0)
            // {
            //    std::cout << "B_mat_c<<" <<mpc_solver.B_mat_c<< endl;
            // }
